...
def main():
    ...
    base_url = "https://www.transfermarkt.com/campeonato-brasileiro-serie-a/gesamtspielplan/wettbewerb/BRA1/saison_id/{season}"
    headers = {
    ...
    for season in range(2024, 2025+1):
        ...
        for i in range(1, 39):
            for row in tabela[i].findAll("tr"):  # para tudo que estiver em <tr>
                ...
                if len(cells) == 7:
                    ht_tag.append(
                        cells[2].findAll(string=True)
                    )  # iterando sobre cada linha
                    result_tag.append(
                        cells[4].findAll(string=True)
                    )  # iterando sobre cada linha
                    at_tag.append(
                        cells[6].findAll(string=True)
                    )  # iterando sobre cada linha
